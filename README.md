...
| **场景信息（给控制算法）** | 每帧把 CARLA 场景里的**周围目标**（交通车、行人、停着的车、电线杆、护栏、路牌等：自车坐标系下的位置、尺寸、朝向、相对速度）、**前方车道**（中心线、车道宽、偏离量、航向差、限速、信号灯）和可选的**传感器数据**（numpy 图像 / 点云 / 雷达）交给你的 Python 控制算法；**碰撞检测**（CarSim 的车在 CARLA 里撞上也不会停）可选停止运行或记录；底部“场景”标签实时显示算法收到的目标（俯视图 + 表格） |
...
```python
    def control(self, exports, t, dt, scene):
        for o in scene["objects"]:            # 周围目标，由近到远
            o["type"]                         # "vehicle" / "walker" / "static"
            o["x"], o["y"]                    # 目标中心，自车坐标系（x 向前、y 向左），m
            o["vx"], o["vy"]                  # 相对自车的速度，m/s
            o["length"], o["width"], o["yaw"], o["distance"], o["moving"]
        lane = scene["lane"]                  # center（前方中心线点）、width、offset、heading_error、speed_limit、traffic_light
        img = scene["sensors"]["cam_front"]["data"]   # “场景信息”页打开“传感器数据”时才有
        return [throttle, brake, steer_sw]
```
在 **场景信息** 页设置给哪些信息（周围目标、地图里的静态物体、前方车道、传感器数据）、范围，以及碰撞时停止运行、只记录还是不检测。坐标原点在自车包围盒中心的地面；场景是上一帧仿真结束时的状态（和真实传感器一样）。
...
| `tests/test_scene.py` | 交给控制算法的场景：直路前方停一辆车，模拟 CarSim 开过去，核对位置、尺寸、相对速度（取 CarSim 的车速）、距离逐帧缩小、前方车道、地图静态物体、碰撞停止并说明撞到什么、只记录时继续运行、传感器数据与采集共用一套传感器、3 个参数的旧算法照常、示例算法跟车并停在前车后面 | 18/18（两种包、两种 CARLA） |
| `tests/test_offline_fixes.py` | 不需要 CARLA：采集文件名检查、容量上限不被排队写入冲破（离上限远时不等待写盘）、传感器挂载失败时恢复原主车、命令行恢复原仿真设置 | 6/6 |
...
| 界面 `--tour` | 自动操作全部页面并截图；用**真实鼠标点击**测试运行 / 暂停 / 单步 / 继续 / 停止、视口按钮、多视图布局与视图内容切换、页签和工程树、底部“场景”标签与筛选、数据浏览（打开 / 逐帧 / 播放）与导出 | 45/45 |

## 已知限制

- 真实 CarSim 的导出变量名和单位需按你的 `.sim` 核对（在 Windows 上运行 CarSim）。
- 车辆由外部动力学驱动时为运动学刚体，不产生碰撞响应（会穿过别的车）；“场景信息”页的碰撞检测按包围盒判断，可选撞上就停止运行。碰撞判断用的是包围盒，树木、建筑不参与；CARLA 地图路面高低起伏而 CarSim 用平路时，把“高度模式”设为“贴合 CARLA 路面”。
