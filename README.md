...
| `tests/test_robustness.py` | 后端抗异常：控制算法在导入或运行时调用 `sys.exit`、输出 NaN，格式错误的请求，不存在的车型（主车和传感器保留），视图建不起来时通知界面，改版客户端连原版服务器时自动用兼容模式，重新连接时清理主车 / 交通 / 视图，非有限数值安全发送 | 18/18（改版客户端 19/19） |
