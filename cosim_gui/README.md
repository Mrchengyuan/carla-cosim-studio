...
- 后端：`tests/test_backend.py` 共 35 项全部通过，覆盖连接、天气、车型尺寸、出生点、主车、交通、传感器（含数据保存）、多视图、录制器、联合仿真（含暂停、单步、继续、完成、启动失败不丢主车）、场景对象、在有交通时连续切换地图；`tests/test_robustness.py` 19 项全部通过（控制算法出错、NaN、格式错误的请求、CARLA 接口自动识别、测量全部车型、重新连接清理）。
