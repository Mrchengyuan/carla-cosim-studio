...
- 后端：`tests/test_backend.py` 共 35 项全部通过，覆盖连接、天气、车型尺寸、出生点、主车、交通、传感器（含数据保存）、多视图、录制器、联合仿真（含暂停、单步、继续、完成、启动失败不丢主车）、场景对象、在有交通时连续切换地图；`tests/test_robustness.py` 全部通过（原版包 28 项、改版包 30 项；控制算法出错、NaN、返回值个数、键盘驾驶配大画面不卡死、格式错误的请求、CARLA 接口自动识别、测量全部车型、主车被 CARLA 删除、交通车被撞飞不卡死、出生点被占、回放、重新连接清理）；`tests/test_carla_restart.py` 4 项（CARLA 运行中退出再重启）。
